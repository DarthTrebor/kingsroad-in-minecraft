...
public class AvoidancePotion implements KRDPotion
{
    private final KRDPotionType type;
    private final Plugin plugin;

    public AvoidancePotion(KRDPotionType type, Plugin plugin)
    {
        this.type = type;
        this.plugin = plugin;
    }
    ...
    public ItemStack createPotion()
    {
        ...
        if (meta != null) {
            ...
            meta.setColor(Color.fromRGB(48, 48, 47));

            String durationText = getDurationStr();
            meta.setLore(List.of(
                    ChatColor.GRAY + "Damage Avoidance " + ChatColor.BLUE + "(" + durationText + ")",
                    ChatColor.GRAY + "Has a chance of " + ChatColor.BLUE + getBoostPercent() + "%" + ChatColor.GRAY + " to avoid damage."
            ));

            NamespacedKey potionTypeKey = new NamespacedKey(plugin, "potionType");
            meta.getPersistentDataContainer().set(potionTypeKey, PersistentDataType.STRING, "avoidance_potion");
    ...
    {
        return type + "_avoidance_potion";
    }

    @Override
    public int getBoostPercent()
    {
        return type == KRDPotionType.NORMAL ? 10 : 20;
    }
    ...
    {
        return (type == KRDPotionType.NORMAL? "" : "Grand ") + "Potion of Avoidance" ;
    }
