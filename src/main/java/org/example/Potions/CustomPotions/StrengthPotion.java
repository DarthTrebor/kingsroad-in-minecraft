...
public class StrengthPotion implements KRDPotion
{
    private final KRDPotionType type;
    private final Plugin plugin;

    public StrengthPotion(KRDPotionType type, Plugin plugin)
    {
        this.type = type;
        this.plugin = plugin;
    }
    ...
    public ItemStack createPotion()
    {
        ...
        if (meta != null)
        {
            meta.setDisplayName(getColorPrefix());
            meta.setColor(Color.fromRGB(184, 24, 13));

            String durationText = (type == KRDPotionType.NORMAL) ? "15:00" : "30:00";
            meta.setLore(List.of(
                    ChatColor.GRAY + "Strength " + ChatColor.BLUE + "(" + durationText + ")",
                    ChatColor.GRAY + "Boosts damage dealt by " + ChatColor.BLUE + getBoostPercent() + "%" + ChatColor.GRAY + '.'
            ));

            NamespacedKey potionTypeKey = new NamespacedKey(plugin, "potionType");
            meta.getPersistentDataContainer().set(potionTypeKey, PersistentDataType.STRING, "strength_potion");
    ...
    {
        return (type == KRDPotionType.NORMAL ? "" : "Grand ") + "Potion of Strength";
    }
